...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=(0, 0),
        ...
        assert 0 <= self.shift_size[0] < self.window_size[0], "shift_size[0] must be in [0, window_size[0])"
        assert 0 <= self.shift_size[1] < self.window_size[1], "shift_size[1] must be in [0, window_size[1])"
...
class BasicLayer(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        ws = to_2tuple(window_size)
        ss = (ws[0] // 2, ws[1] // 2)

        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=ws,
                                 shift_size=(0, 0) if (i % 2 == 0) else ss,
                                 mlp_ratio=mlp_ratio,
...
class SwinTransformerBackbone(nn.Module):
    def __init__(self, img_size=(128, 512), patch_size=4, in_chans=3,
    ...
    def forward(self, x):
        ...
        # Reshape to (B, C, H, W) for compatibility with some decoders
        B, L, C = x.shape
...
if __name__ == '__main__':
    ...
    # This configuration is similar to Swin-T but adapted for non-square images
    ...
    # Batch size = 2, Channels = 1, Height = 128, Width = 512
    dummy_input = torch.randn(2, 1, img_size[0], img_size[1])

    # Get the output from the model
    output = model(dummy_input)

    # Print the output shape
    print(f"SwinTransformerBackbone test")
    print(f"Input shape: {dummy_input.shape}")
    print(f"Output shape: {output.shape}")

    # Expected output shape calculation:
    # Patch size = 4x4
    # Initial patches resolution = (128/4, 512/4) = (32, 128)
    # Number of downsampling stages = num_layers - 1 = 3
    # Final patches resolution = (32 / 2^3, 128 / 2^3) = (4, 16)
    # Final channels = embed_dim * 2^3 = 96 * 8 = 768
    # Expected shape: (Batch, Channels, Height, Width) = (2, 768, 4, 16)